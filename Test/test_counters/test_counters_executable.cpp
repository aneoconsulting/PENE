...
void main(int argc, char* argv[])
{
  ...
  if (precision.compare("float") == 0)
  {
    ...
    std::cout << operation << "_" << precision << "_" << mode << " - result: " << reinterpret_cast<float*>(&accu)[0] << std::endl;
  }
  else if(precision.compare("double") == 0)
  {
    ...
    std::cout << operation << "_" << precision << "_" << mode << " - result: " << reinterpret_cast<double*>(&accu)[0] << std::endl;
  }
}